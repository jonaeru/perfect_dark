	...
	*dst_portalvtxs = *dstpos;
	...
	while (1) {
		...
		for (int j = 0; j < host_pvertices->count; j++) {
			//host_pvertices->vertices[j].x = srctodst32(n64_pvertices->vertices[j].x);
			//host_pvertices->vertices[j].y = srctodst32(n64_pvertices->vertices[j].y);
			//host_pvertices->vertices[j].z = srctodst32(n64_pvertices->vertices[j].z);
...
static void convertPrimaryLights(u8 *dst, u32 *dstpos, u8 *src, u32 *srcpos, u32 end)
{
	...
	for (int i = 0; i < num_lights; i++) {
		...
		for (int j = 0; j < 4; j++) {
			host_lights[i].bbox[j].x = srctodst16(n64_lights[i].bbox[j].x);
			host_lights[i].bbox[j].y = srctodst16(n64_lights[i].bbox[j].y);
			host_lights[i].bbox[j].z = srctodst16(n64_lights[i].bbox[j].z);
		}
	}
...
static u32 convertRoomGfxData(u8 *dst, u8 *src, u32 infsize, u32 src_ofs)
{
	...
	while ((intptr_t)(curpos_src + sizeof(struct n64_roomblock)) <= endpos) {
		...
		addMarker(curpos_src + src_ofs, curpos_dst + dst_roomoffset);
	...
	for (size_t i = 0; i < numgdls; i++) {
		addMarker(gdls_addr[i] + src_ofs, curpos_dst + dst_roomoffset);
