...
u32 obj_size(struct n64_defaultobj *obj)
{
	switch (obj->type) {
...
void conv_default_obj_hdr(struct defaultobj* dstobj, struct n64_defaultobj* srcobj)
{
...
void conv_default_obj(struct defaultobj* dstobj, struct n64_defaultobj* srcobj)
{
	PD_CONV_VAL(dstobj->extrascale, srcobj->extrascale);
...
u32 convert_props(u8* dst, u8* src)
{
	u8* start = dst;
...
static int convert_setup(u8 *dst, u8 *src, u32 srclen)
{
